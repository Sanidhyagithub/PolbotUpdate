...
def already_alerted_for_target(market: dict, target: datetime) -> bool:
    if state["last_alerted_date"] != target_date_key(target):
        return False

    current_identity = get_market_identity(market)
    return current_identity is None or current_identity == state["last_alerted_market"]

def mark_market_alerted(market: dict, target: datetime):
    state["last_alerted_date"] = target_date_key(target)
    state["last_alerted_market"] = get_market_identity(market)
...
async def check_market_once(session: aiohttp.ClientSession, target: datetime):
    ...
    for m in markets:
        if market_matches_target(m, target):
...
def make_status_embed() -> discord.Embed:
    ...
    embed.add_field(name="Last Alerted Date", value=state["last_alerted_date"] or "—",        inline=True)
...
async def watch_loop():
    ...
    target  = get_target_date()
    month   = MONTHS[target.month - 1].capitalize()
    state["current_target_date"] = target_date_key(target)
    ...
    while not client.is_closed():
        try:
            ...
            if state["current_target_date"] != target_key:
                state["current_target_date"] = target_key
                state["last_check_result"] = f"🗓️ Watching {target.strftime('%B %d, %Y')}"
                log.info(f"Rolled over to target market date {target_key}")
            ...
            if error:
                ...
                if already_alerted_for_target(market, target):
                    state["last_check_result"] = "ℹ️ Already alerted for this date"
