...
3. Either fill in BOT_TOKEN and CHANNEL_ID below, or set
   POLYMARKET_BOT_TOKEN and POLYMARKET_CHANNEL_ID in your shell
...
WATCH_DAYS_AHEAD       = 7         # Watch the next N calendar days
...
def load_config():
    bot_token = os.getenv("POLYMARKET_BOT_TOKEN", BOT_TOKEN).strip()
    channel_raw = os.getenv("POLYMARKET_CHANNEL_ID", str(CHANNEL_ID)).strip()
    ...
    try:
        ...
        channel_id = None
        missing.append("Discord channel ID (must be an integer)")

    return bot_token, channel_id, missing

BOT_TOKEN, CHANNEL_ID, CONFIG_ERRORS = load_config()
...
def get_target_dates(days_ahead: int = WATCH_DAYS_AHEAD) -> list[datetime]:
    now = datetime.now(IST)
    return [now + timedelta(days=offset) for offset in range(1, days_ahead + 1)]

def format_target_window(targets: list[datetime]) -> str:
    return ", ".join(target.strftime("%b %d") for target in targets)

def target_date_key(target: datetime) -> str:
    return target.strftime("%Y-%m-%d")

def build_events_urls() -> list[str]:
    urls = []
    limit = 200
    for offset in (0, 200, 400):
        urls.append(
            f"{GAMMA_EVENTS_API}?active=true&closed=false&limit={limit}&offset={offset}&order=start_date&ascending=true"
        )
    return urls
...
def build_expected_questions(target: datetime) -> set[str]:
    month = MONTHS[target.month - 1].capitalize()
    return {
        normalize_text(f"Highest temperature in Lucknow on {month} {target.day}"),
        normalize_text(f"Highest temperature in Lucknow on {month} {target.day} {target.year}"),
    }

def build_expected_slug_prefixes(target: datetime) -> tuple[str, ...]:
    month = MONTHS[target.month - 1]
    day = target.day
    year = target.year
    return (
        f"highest-temperature-in-lucknow-on-{month}-{day}",
        f"highest-temperature-in-lucknow-on-{month}-{day}-{year}",
    )

def get_market_identity(market: dict) -> Optional[str]:
    for key in ("id", "eventId", "conditionId", "slug", "eventSlug"):
...
def market_matches_target(market: dict, target: datetime) -> bool:
    question = normalize_text(market.get("title") or market.get("question") or "")
    slug = (market.get("slug") or market.get("eventSlug") or "").lower().strip()

    if question and question in build_expected_questions(target):
        return True

    for prefix in build_expected_slug_prefixes(target):
        if slug == prefix or slug.startswith(f"{prefix}-"):
            return True

    return False

def format_market_debug_line(market: dict) -> str:
    question = market.get("title") or market.get("question") or "—"
    slug = market.get("slug") or market.get("eventSlug") or "—"
    return f"{question} | slug={slug}"

def already_alerted_for_target(market: dict, target: datetime) -> bool:
    alerted_markets = state["alerted_markets"]
    target_key = target_date_key(target)
    if target_key not in alerted_markets:
        return False

    current_identity = get_market_identity(market)
    return current_identity is None or current_identity == alerted_markets[target_key]

def mark_market_alerted(market: dict, target: datetime):
    target_key = target_date_key(target)
    state["alerted_markets"][target_key] = get_market_identity(market)

    active_target_keys = {target_date_key(target) for target in get_target_dates()}
    state["alerted_markets"] = {
        date_key: market_id
        for date_key, market_id in state["alerted_markets"].items()
        if date_key in active_target_keys
    }
...
async def check_markets_once(session: aiohttp.ClientSession, targets: list[datetime]):
    unique_markets = {}

    for url in build_events_urls():
        ...
        for event in events:
            ...
            if normalized_event.get("slug") and not normalized_event.get("eventSlug"):
                normalized_event["eventSlug"] = normalized_event["slug"]
            identity = get_market_identity(normalized_event) or f"title:{normalized_event.get('title', '')}"
            unique_markets[identity] = normalized_event

    found_markets = {}
    for target in targets:
        for market in unique_markets.values():
            if market_matches_target(market, target):
                log.info(
                    f"Market found for {target_date_key(target)}: {market.get('title') or market.get('question')}"
                )
                found_markets[target_date_key(target)] = market
                break

    nearby_lucknow_markets = [
        format_market_debug_line(market)
        for market in unique_markets.values()
        if "lucknow" in normalize_text(market.get("title") or market.get("question") or "")
        or "lucknow" in (market.get("slug") or market.get("eventSlug") or "").lower()
    ][:5]
    if nearby_lucknow_markets and len(found_markets) != len(targets):
        log.warning(
            "Some target markets were not found. Nearby Lucknow markets returned by API: %s",
            " || ".join(nearby_lucknow_markets),
        )
    elif not nearby_lucknow_markets and len(found_markets) != len(targets):
        log.warning("Target markets were not found and no Lucknow markets were returned by the API search set.")

    return found_markets


async def check_markets_with_retry(targets: list[datetime]):
    """Returns (markets_by_date, error_string_or_None)"""
    last_error = None

    async with aiohttp.ClientSession() as session:
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                result = await check_markets_once(session, targets)
                return result, None
...
def make_found_embed(market: dict, target: datetime) -> discord.Embed:
    ...
    month = MONTHS[target.month - 1].capitalize()
    title = market.get("title") or market.get("question") or "Lucknow Temperature Market"
    embed = discord.Embed(
...
def make_status_embed() -> discord.Embed:
    ...
    embed.add_field(name="Watching Window",  value=format_target_window(targets),            inline=False)
    embed.add_field(name="Markets Found",    value=str(state["markets_found"]),               inline=True)
    embed.add_field(name="Alerted Dates",    value=", ".join(sorted(state["alerted_markets"].keys())) or "—", inline=False)
...
async def watch_loop():
    ...
    targets = get_target_dates()
    state["current_target_dates"] = [target_date_key(target) for target in targets]
    last_hb = datetime.now(IST)

    await ch.send(embed=discord.Embed(
        title       = "🤖 Polymarket Watcher Online",
        description = f"Watching for **Lucknow temperature markets** for the next **{WATCH_DAYS_AHEAD} days**.\n"
                      f"Window: **{format_target_window(targets)}**.\n"
    ...
    while not client.is_closed():
        try:
            ...
            if (datetime.now(IST) - last_hb).total_seconds() >= HEARTBEAT_HOURS * 3600:
                ...
                target_hb = get_target_dates()
                embed.add_field(name="Watching",     value=format_target_window(target_hb), inline=False)
                embed.add_field(name="Uptime",       value=uptime_str(),                inline=True)
                embed.add_field(name="Total Checks", value=str(state["total_checks"]),  inline=True)
                embed.timestamp = datetime.utcnow()
                await ch.send(embed=embed)
            ...
            targets                  = get_target_dates()
            target_keys              = [target_date_key(target) for target in targets]

            if state["current_target_dates"] != target_keys:
                state["current_target_dates"] = target_keys
                state["last_check_result"] = f"🗓️ Watching {format_target_window(targets)}"
                log.info("Rolled over to target market window %s", ", ".join(target_keys))

            markets_by_date, error = await check_markets_with_retry(targets)
            ...
            elif markets_by_date:
                state["consecutive_errors"] = 0
                state["successful_checks"]  += 1
                state["last_check_result"]   = f"✅ {len(markets_by_date)} market(s) found in window"

                new_alerts = 0
                for target in targets:
                    target_key = target_date_key(target)
                    market = markets_by_date.get(target_key)
                    if not market:
                        continue

                    if already_alerted_for_target(market, target):
                        log.info(f"Skipping duplicate alert for {target_key}")
                        continue
                    ...
                    mark_market_alerted(market, target)
                    log.info(f"Notification sent for {target_key}")

                if new_alerts == 0:
                    state["last_check_result"] = "ℹ️ All found markets were already alerted"
                else:
                    state["last_check_result"] = f"✅ Sent {new_alerts} new alert(s)"

            else:
                state["consecutive_errors"] = 0
                state["successful_checks"]  += 1
                state["last_check_result"]   = "✅ No live markets found in watch window"
...
async def on_message(message):
    ...
    if cmd == "!status":
        ...
        if error:
            ...
            for target in targets:
                market = markets_by_date.get(target_date_key(target))
                if market:
                    await message.channel.send(embed=make_found_embed(market, target))
        else:
            await message.channel.send(
                f"❌ No Lucknow markets are live in the next **{WATCH_DAYS_AHEAD} days** "
                f"({format_target_window(targets)})."
            )
        ...
        state["consecutive_errors"] = 0
        await message.channel.send(
            f"▶️ Resumed! Polling every {POLL_INTERVAL_SECONDS} seconds for the next {WATCH_DAYS_AHEAD} days."
        )
...
async def on_ready():
    ...
    watch_task = state.get("watch_task")
    if watch_task and not watch_task.done():
        log.info("Watch loop already running; skipping duplicate start.")
        return

    state["watch_task"] = client.loop.create_task(watch_loop())
...
if __name__ == "__main__":
    if CONFIG_ERRORS:
        ...
        print("\n  Set either:")
        print("  - BOT_TOKEN and CHANNEL_ID in the script, or")
        print("  - POLYMARKET_BOT_TOKEN and POLYMARKET_CHANNEL_ID in your shell")
        print("="*50 + "\n")
    else:
