...
def build_event_slug(target: datetime) -> str:
    ...
    return f"highest-temperature-in-lucknow-on-{month}-{target.day}-{target.year}"

def build_event_url(target: datetime) -> str:
    return f"{GAMMA_EVENTS_API}/slug/{build_event_slug(target)}"
...
async def check_markets_once(session: aiohttp.ClientSession, targets: list[datetime]):
    ...
    for target in targets:
        url = build_event_url(target)
        log.info(f"Polling: {url}")

        async with session.get(url, timeout=aiohttp.ClientTimeout(total=15)) as resp:
            if resp.status == 404:
                missing_targets.append(target_date_key(target))
                continue
            if resp.status == 429:
        ...
        event = dict(data)
        if event.get("slug") and not event.get("eventSlug"):
            event["eventSlug"] = event["slug"]

        if market_matches_target(event, target):
            log.info(
                "Market found for %s: %s",
                target_date_key(target),
                event.get("title") or event.get("question"),
            )
            found_markets[target_date_key(target)] = event
        else:
            log.warning(
                "Slug lookup returned a non-matching event for %s: %s",
                target_date_key(target),
                format_market_debug_line(event),
            )

    if missing_targets:
        log.info("No event found yet for target dates: %s", ", ".join(missing_targets))
