...
def build_search_urls(target: datetime) -> list[str]:
    month = MONTHS[target.month - 1].capitalize()
    base_queries = [
        f"Highest temperature in Lucknow on {month} {target.day}",
        f"Highest temperature in Lucknow",
        f"Lucknow {month} {target.day}",
        "Lucknow",
    ]
    ...
    for query in base_queries:
        ...
        urls.append(f"{GAMMA_API}?{urlencode(params)}")
    return urls
...
def market_matches_target(market: dict, target: datetime) -> bool:
    ...
    slug = (market.get("slug") or "").lower().strip()
...
async def check_markets_once(session: aiohttp.ClientSession, targets: list[datetime]):
    ...
    for target in targets:
        search_urls.extend(build_search_urls(target))

    unique_markets = {}

    for url in dict.fromkeys(search_urls):
        ...
        for market in markets:
            identity = get_market_identity(market) or f"question:{market.get('question', '')}"
            unique_markets[identity] = market

    found_markets = {}
    for target in targets:
        for market in unique_markets.values():
            if market_matches_target(market, target):
                log.info(f"Market found for {target_date_key(target)}: {market.get('question')}")
    ...
    nearby_lucknow_markets = [
        ...
        for market in unique_markets.values()
        if "lucknow" in normalize_text(market.get("question") or "") or "lucknow" in (market.get("slug") or "").lower()
    ][:5]
